...
class Meta(type):
    def __new__(mcs, name, bases, dct):
        print("Criando a instância[METACLASS]")
        cls = super().__new__(mcs, name, bases, dct)
        cls.attr = "Fernando"
        cls.__repr__ = meu_repr

        if "falar" not in cls.__dict__ or not callable(cls.__dict__["falar"]):
            raise NotImplementedError("Classe deve ter o método falar")
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if "nome" not in instancia.__dict__:
            raise NotImplementedError("Classe deve ter o método nome")
        return instancia
...
class Pessoa(metaclass=Meta):
    ...
    def falar(self):
        print("Falando alguma coisa")

    def dizer(self):
        print("Estou falando com voce pelo chat de conversa multipla")


p1 = Pessoa("Fernando", 36)
print(p1.__repr__)
p1.falar()
p1.dizer()
